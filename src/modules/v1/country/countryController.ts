...
class CountryController {
  ...
  public create = async (req: Request, res: Response) => {
    ...
    const result = await this.CountryUtils.countryAdd(applicationData);
    if (result.code === Constants.FAIL_CODE) {
      return res.status(result.code).json(result);
    } else {
      const response = {
        statusCode: Constants.SUCCESS_CODE,
        message: req.t('ADD_COUNTRY'),
      };
  ...
  public getAll = async (req: Request, res: Response) => {
    try {
      const result = await this.CountryUtils.countrygetAll();
      if (result) {
  ...
  public findById = async (req: Request, res: Response) => {
    try {
      ...
      const result = await this.CountryUtils.countryfindById(id);
      if (result?.message) {
        return res.status(result.statusCode).json(ResponseBuilder.badRequest(req.t(result?.message)));
      }
      return res.status(200).json(ResponseBuilder.data(result, req.t('COUNTRY_FINDBYID')));
    } catch (error) {
      return res.status(500).send({
        error: error,
        message: req.t('ERR_INTERNAL_SERVER'),
  ...
  public Update = async (req: Request, res: Response) => {
    try {
      ...
      if (result) {
        const applicationData = req.body;
        const update = await this.CountryUtils.countryupdate(id, applicationData);
        return res.status(200).json(ResponseBuilder.data(update, req.t('DATA_UPDATE')));
      } else {
        return res.status(Constants.FAIL_CODE).json(ResponseBuilder.badRequest(req.t('COUNTRY_NOT_FOUND')));
      }
    } catch (error) {
      return res.status(500).send({
        error: error,
  ...
  public Delete = async (req: Request, res: Response) => {
    try {
      const id = req.params.id;
      const result = await this.CountryUtils.countryDelete(id);
      if (result?.message) {
        return res.status(result.statusCode).json(ResponseBuilder.badRequest(req.t(result?.message)));
      }
